...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; bg: string }> = {
    success: { label: "SUCCESS", bg: "bg-success" },
    fail: { label: "FAILED", bg: "bg-destructive" },
    early_stop: { label: "EARLY STOP", bg: "bg-warning" },
    unknown: { label: "UNKNOWN", bg: "bg-muted-foreground" },
  };
  const cfg = config[status] ?? config.unknown;
  return (
    <span className={`${cfg.bg} text-background text-[9px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-sm`}>
      {cfg.label}
    </span>
  );
}
